...
import { Component, OnInit, Input, Output, EventEmitter, ChangeDetectorRef, ViewChild, ViewChildren, ElementRef, HostListener, OnDestroy } from '@angular/core';
import { GlobalService } from 'src/app/services/global.service';
import { ActivatedRoute, Router } from '@angular/router';
import { FileUploader, FileItem, ParsedResponseHeaders, FileUploadModule } from 'ng2-file-upload';
import { NgbModal, NgbActiveModal, NgbModalRef, ModalDismissReasons, NgbModule } from '@ng-bootstrap/ng-bootstrap';
import { environment } from '../../../environments/environment';
import { WebcamImage, WebcamInitError, WebcamUtil, WebcamModule } from 'ngx-webcam';
import { Subject, Observable, Subscription, defer, from, interval } from 'rxjs';
...
@Component({
  selector: 'app-signature-photograph',
  templateUrl: './signature-photograph.component.html',
  styleUrls: ['./signature-photograph.component.scss'],
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, PipesModule, SharedModule, TranslateModule, NgbModule, ImageCropperModule, WebcamModule, FileUploadModule, InPersonVideoComponent, ControlMessagesComponent, SignaturePadComponent]
})
export class SignaturePhotographComponent implements OnInit, OnDestroy {
  @Input() userInfo: any;
  ...
  ngOnInit(): void {
    ...
    if (this.verifiedSteps.mf_app) {
      ...
      if (this.verifiedSteps.hasOwnProperty('isNominatedVerified') && !this.verifiedSteps?.isNominatedVerified) {
        ...
        if (this.verifiedSteps.hasOwnProperty('isDeclaration') && !this.verifiedSteps?.isDeclaration) {
        ...
        } else if (this.verifiedSteps.hasOwnProperty('isPaymentRequired') && !this.verifiedSteps?.isPaymentCompleted && this.verifiedSteps?.isPaymentRequired) {
          this.ref.detectChanges();
          setTimeout(() => {
            this.global.redirectToLastVerifiedToastr(this.verifiedSteps);
          }, 500);
    ...
    if (this.global?.AINEXTSelfieMode) {
  ...
  ngAfterViewInit() {
    this.global.logEventUserExperior("Landing on Signature Photograph Screen");
      ...
      if (result.type === 1 && result.loaded && result.total) {
        ...
        if (result.body.success) {
          if (this.nameOfDocument === 'photograph') {
            ...
            /************************ GA TAG PUSH : STOP ********************/
            this.global.logEventUserExperior("Uploaded photograph successfully");
          } else {
            /************************ GA TAG PUSH : START ********************/
            let utmSource = '';
            let utmMedium = '';
            this.global.setPushToGTM({
              'event': 'confirm_signature_success',
              'Page_Location': this.router.url.split('?')[0],
              'Page_referrer': this.global?.previousUrl,
              'CTA_text': 'Save & Continue',
              'FormID': 'form id',
              'user_id': `confirm_signature_success${Math.random()}`,
              'UTM_source': utmSource ? utmSource : '',
              'UTM_medium': utmMedium ? utmMedium : '',
            });
            /************************ GA TAG PUSH : STOP ********************/
            this.global.logEventUserExperior("Uploaded signature successfully");
          }
  ...
  /****************** AINEXT CODE START *********/
  getAINEXTToken() {
    // const tokn = `eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJqdGkiOiIzZjk1ZGYyOS1lNGY3LTRjMTItOTM5Mi0wODRiN2IxZTNkZTkiLCJyZWZpZCI6IjEyMjM0IiwiZXhwIjoxNzE2ODgxMTk2LCJpc3MiOiJkNXdhenBlcHpmLmV4ZWN1dGUtYXBpLmFwLXNvdXRoLTEuYW1hem9uYXdzLmNvbSIsImF1ZCI6ImQ1d2F6cGVwemYuZXhlY3V0ZS1hcGkuYXAtc291dGgtMS5hbWF6b25hd3MuY29tIn0.npF04oQcLpYhjUlJStGWiiPqQUg_bLjxBAamUlKt5Hk`;
    // window.AinxtSDK.Initialize(tokn, this.handleAINEXTSDKData);
    const obj = {};
    this.global.getAINEXTToken(obj, true).subscribe((res: any) => {
      if (res?.success) {
        if (res?.result?.Token) {
          this.AINXTAUTHToken = res?.result?.Token;
  ...
  initializeAiNxt() {
    if (this.AINXTAUTHToken) {
      this.getGeoLocation();
      // console.log('this.AINXTAUTHToken', this.AINXTAUTHToken);
      window.AinxtSDK.Initialize(this.AINXTAUTHToken, this.handleAINEXTSDKData);
    } else {
      this.getGeoLocation();
      const obj = {};
      this.global.getAINEXTToken(obj, true).subscribe((res: any) => {
    ...
    this.dashboardService.aiNxtLocation(obj).subscribe((res: any) => {
      if (res?.success) {
        if (res?.result?.country && res?.result?.country?.toUpperCase() !== `INDIA`) {
          ...
          this.liveWebcam = false;
          this.modalRef.close();
          document.getElementById("usercancletheliveliness").click();
  ...
  */
  ...
  openDocumentPopupModal(nameOfDocument: any, nameOfModalTitle: any, documentStatus = '', allItem: any = '') {
    ...
    this.cropperQuality = 50;
    if (this.nameOfDocument === 'photograph') {
      this.cropperQuality = 100;
    }
    ...
    this.modalRef = this.modalService.open(this.fileuploadAadharpopup, { windowClass: 'mymodal modal-fullscreen', centered: true, size: 'lg', backdrop: 'static', keyboard: false });
  ...
  public onFileSelected(event: EventEmitter<File[]>, allItem) {
    this.allowedMimeType = allItem.mime_type;
    const objMime = { name: "mimeType" }
    // this.onWhenAddingFileFailed('', objMime, '');
    const file: File = event[0];
    this.isUploadFile = allItem.isUploadFile;
    this.isUseCamera = allItem.isUseCamera;
    // console.log('this.allowedMimeType', this.allowedMimeType);
    this.maxUploadLimit = allItem.remaining_count;
    this.isDocumentVerified = (allItem?.document_status === 'verified') ? true : false;
    this.openDocumentPopupModal(allItem?.document_name, allItem?.document_modal_title, allItem?.document_status, allItem);
    ...
    this.dashboardService.uploadDocument(uploadParam).subscribe((result: any) => {
      if (result.type === 1 && result.loaded && result.total) {
        ...
        if (result.body.success) {
          ...
          /************************ GA TAG PUSH : STOP ********************/
          this.global.logEventUserExperior("Verified photograph successfully");
          this.manageResultAfterUploadingFiles(result.body.result, true);
          this.modalRef.close();
        }
      }
  ...
  handleHypervergeSelfie() {
    if (this.webCamMediaList?.length < 2) {
      this.webCamMediaList.push(this.faceImageBase64);
      this.webCamShowMediaBig(0);
    } else {
      this.global.errorToastr(this.translate.instant('cant_upload_max'));
      return true;
  ...
  enableSignaturePad() {
    ...
    this.viewSectionOfImage = false;
    this.fileUploading = false;
    this.signaturePadLoading = false;
  }
  clearImage() {
    console.log('clear image');
  }

  async showImage(baseImage: any) {
    // console.log('data', baseImage);
    this.signatureImage = baseImage;
    this.isEnableSignaturePad = true;
    // const resizedImage: any = await this.resizeImage(this.signatureImage, 300, 300);
    // console.log('resizedImage', resizedImage);
    // this.signatureImage = resizedImage;
    this.onResize();
  ...
  onResize() {
    // console.log('class--length', $('.mymodal').find(".modal-content").length)
    if ($('.mymodal').find(".modal-content").length > 0) {
      const signaturePadwidth = $('.mymodal').find('.modal-content').width();
      this.signatureWidth = signaturePadwidth - 50;
      const signaturePadHeight = window.innerHeight - 200;
      this.signatureHeight = signaturePadHeight;
      if (signaturePadHeight > 400) {
        this.signatureHeight = 400;
      }
    } else {
  ...
  openSinaturePopupModal(nameOfDocument: any, nameOfModalTitle: any, documentStatus = '', allItem: any = '') {
    ...
    this.modalRef = this.modalService.open(this.fileuploadSignaturepopup, { centered: true, windowClass: 'mymodal', size: 'lg', backdrop: 'static', keyboard: false });
    setTimeout(() => {
      this.onResize();
    }, 1000);
  ...
  */
  ...
  checkStepToRedirect() {
    if (this.verifiedSteps?.isSignaturePhotographUploaded) {
