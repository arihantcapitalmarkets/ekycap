...
@Component({
  selector: 'app-futureoption',
  templateUrl: './futureoption.component.html',
  styleUrls: ['./futureoption.component.scss'],
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, PipesModule, SharedModule, TranslateModule, NgbModule, ImageCropperModule, WebcamModule, FileUploadModule]
})
export class FutureoptionComponent implements OnInit {
  ...
  getDocumentAvailableList = [
    {
      "text": "Required only for Future & Options",
      "isUploadFile": true,
      "isUseCamera": false,
      "document_status": "not_uploaded",
      "document_name": "income_proof",
      "mime_type": [
        "image/png",
        "image/jpeg",
        "image/jpg",
      ...
      "is_visible": true,
      "document_button_lable": "Income Proof",
      "document_modal_title": "Income Proof",
      "total_upload": 1,
      "is_optional": false,
      "remaining_count": 1,
      "type": "document",
      "isDocsProofRequired": true,
      "docs_proof": [
        {
          "id": 1,
          "name": "Bank Account statement for the last 6 months"
        },
        {
          "id": 2,
          "name": "Copy of ITR acknowledgement"
        },
        {
          "id": "3",
          "name": "The latest salary slip"
        }
      ],
      "isBankDoc": false,
      "detailText": "<p><strong>Want to trade in derivatives?&nbsp;</strong><br />We need some additional information to activate this segment.</p>",
      "canUpload": true
    }
  ];
  ...
  submitDocumentUploadModalCropped() {
    ...
    uploadParam.append('document_name', 'income_proof');
    ...
    this.loader = true;
    this.dashboardService.uploadIncomeProof(uploadParam).subscribe((result: any) => {
  ...
  ngOnInit(): void {
    ...
    if (this.getCookie()) {
      this.serviceVarService.setValue(true);
    }

  }
  incomeProofObj: any;
  ngAfterViewInit(): void {
    this.incomeProofObj = this.getDocumentAvailableList[0];
    // if (this.incomeProofObj) {
    //   this.openDocumentPopupModal(this.incomeProofObj?.document_name, this.incomeProofObj?.document_modal_title, this.incomeProofObj?.document_status, this.incomeProofObj);
    // }
  }

  getCookie() {
    return this.cookie.get('user_auth_token');
  }
